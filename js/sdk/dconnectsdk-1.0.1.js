...
var dConnect = (function(parent, global) {
     ...
     * 画面へと進ませる。
     * @memberOf dConnect
     * @param {String=} redirectURL リダイレクト先URL。省略された場合は現在表示されているページがリダイレクト先となる。
     * @param {String} appId Apple App ID。iOSの場合で、かつDeviceConnect対応アプリ上で動作していなかった場合は指定したアプリのページを表示する。Androidの場合にはこの引数は無視される。
     * @param {function} success_cb 成功時コールバック。
     * @param {function} error_cb 失敗時コールバック。
     */
    var checkDeviceConnect = function(redirectURL, appId, success_cb, error_cb) {
        this.getSystemInfo(function(status, headers, data) {
            ...
            if (status === 200) {
                ...
                if (obj.result == parent.constants.RESULT_OK) {
                    ...
                    error_cb("Device Connect was not found; it seems another server is running instead.");
                }
            } else if (status === 0) {
